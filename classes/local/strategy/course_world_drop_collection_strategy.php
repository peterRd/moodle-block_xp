...
use block_xp\local\logger\reason_occurance_indicator;
use block_xp\local\reason\drop_collected_reason;
...
class course_world_drop_collection_strategy implements drop_collection_strategy {
     ...
     */
    public function __construct(course_user_state_store $store, reason_collection_logger $logger) {
        $this->logger = $logger;
        $this->store = $store;
    }
    ...
    public function can_collect(drop $drop, $userid) {
        ...
        $event = new drop_collected_reason($drop->get_id());
        return !$this->logger->has_reason_happened_since($userid, $event);
    }
}
